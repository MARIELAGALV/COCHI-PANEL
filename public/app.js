...
function editContentItem(groupIndex,itemIndex=null){
  ...
  const playbackSources=(Array.isArray(cur.playbackSources)&&cur.playbackSources.length?cur.playbackSources.map(x=>({url:String(x?.url||'').trim(),headers:(x?.headers&&typeof x.headers==='object')?x.headers:{},type:String(x?.type||x?.tipo||'auto').toLowerCase(),drm_scheme:String(x?.drm_scheme||'').toLowerCase(),keys:Array.isArray(x?.keys)?x.keys:[],drm_license_url:String(x?.drm_license_url||x?.license_url||'').trim(),drm_license_headers:(x?.drm_license_headers&&typeof x.drm_license_headers==='object')?x.drm_license_headers:((x?.license_headers&&typeof x.license_headers==='object')?x.license_headers:{})})):[{url:String(cur.uri||'').trim(),headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{},type:streamType,drm_scheme:drmScheme,keys:Array.isArray(cur.keys)?cur.keys:[],drm_license_url:existingLicenseUrl,drm_license_headers:existingLicenseHeaders},...legacyBackups.map(url=>({url,headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{},type:'auto',drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{}}))]).filter((x,i)=>x.url||i===0);
  ...
  const renderPlaybackSources=()=>{
    ...
    $$('.ci-playback-drm').forEach(sel=>sel.onchange=()=>{const row=sel.closest('[data-playback-source]'),drm=sel.value||'';row.querySelector('.ci-source-clearkey').style.display=drm==='clearkey'?'grid':'none';row.querySelector('.ci-source-widevine').style.display=drm==='widevine'?'grid':'none';});$$('.ci-playback-type').forEach(sel=>sel.onchange=()=>{if(sel.value!=='remote_playlist')return;const row=sel.closest('[data-playback-source]'),drmSel=row.querySelector('.ci-playback-drm');if(drmSel){drmSel.value='';drmSel.dispatchEvent(new Event('change'));}});
  ...
  if(isTv){renderPlaybackSources();if($('#ciAddPlaybackSource'))$('#ciAddPlaybackSource').onclick=()=>{playbackSources.push({url:'',headers:{},type:'auto',drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{}});renderPlaybackSources();};}
  ...
  $('#contentItemForm').onsubmit=async e=>{
    ...
    try{
      ...
      if(isTv){
        ...
        let selectedOriginalIndex=Number($('input[name="ciActivePlaybackSource"]:checked')?.value);if(!Number.isInteger(selectedOriginalIndex)||selectedOriginalIndex<0)selectedOriginalIndex=activePlaybackSource;
