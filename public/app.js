...
function editContentItem(groupIndex,itemIndex=null){
  ...
  openModal(`<div class="content-editor-head"><div><h3>${itemIndex===null?'Agregar':'Editar'} contenido</h3><p class="muted small">Edición ampliada: cada fuente de TV conserva su URL, headers, formato y DRM de forma independiente.</p></div><button type="button" class="ghost compact-close" data-close>✕</button></div><form id="contentItemForm" class="content-editor-form">
    <div class="content-editor-grid">
      <section class="content-editor-column">
        ...
        ${isTv?`<div class="content-icon-edit-row"><div class="content-icon-preview-shell"><img id="ciIconPreview" ${cur.icon?`src="${esc(cur.icon)}"`:''} alt="Miniatura del canal" ${cur.icon?'':'hidden'}><span id="ciIconPreviewEmpty" class="muted tiny" ${cur.icon?'hidden':''}>SIN IMAGEN</span></div><label>Icono / carátula<input id="ciIcon" value="${esc(cur.icon||'')}" placeholder="https://..."><span class="muted tiny">La imagen se muestra acá en miniatura para revisarla antes de actualizar.</span></label></div>`:`<label>Icono / carátula<input id="ciIcon" value="${esc(cur.icon||'')}" placeholder="https://..."></label>`}
  ...
  if(isTv&&$('#ciIcon')){
    ...
    const setIconPreview=()=>{const url=String(iconInput.value||'').trim();if(!preview||!empty)return;if(!url){preview.hidden=true;preview.removeAttribute('src');empty.hidden=false;return;}empty.hidden=true;preview.hidden=false;preview.src=url;};
    if(preview)preview.onerror=()=>{preview.hidden=true;if(empty)empty.hidden=false;};
    iconInput.addEventListener('input',()=>{clearTimeout(iconPreviewTimer);iconPreviewTimer=setTimeout(setIconPreview,350);});
