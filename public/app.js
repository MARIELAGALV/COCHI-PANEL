...
function editContentItem(groupIndex,itemIndex=null){
  ...
  if(isTv){renderPlaybackSources();if($('#ciAddPlaybackSource'))$('#ciAddPlaybackSource').onclick=()=>{syncPlaybackSourcesFromDom();playbackSources.push({url:'',headers:{},type:'auto',drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{},clearkey_input_order:'kid_key'});renderPlaybackSources();};}
      ...
      if(isTv){
        ...
        for(const row of sourceRows){
          ...
          if(sourceDrm==='clearkey'){
            ...
            if(!keys.length)throw new Error(`ClearKey en URL ${originalIndex+1}: cargá al menos un par KID:KEY o KEY:KID.`);
        ...
        if(active.drm_scheme==='clearkey'){obj.drm_scheme='clearkey';obj.keys=active.keys;obj.clearkey_input_order=active.clearkey_input_order==='key_kid'?'key_kid':'kid_key';}
