...
function openHomeBannerImageEditor(slot){
  ...
  openModal(`<div class="banner-crop-head"><div><h3>Ajustar imagen ${slot}</h3><p class="muted small">Mové la imagen con el mouse o el dedo. En <b>LIBRE</b> podés cambiar ancho y alto por separado para estirar o achicar solamente la zona que necesites.</p></div><span class="badge">16:9</span></div><div class="banner-crop-stage"><canvas id="bannerCropCanvas" width="1280" height="720"></canvas><div id="bannerCropLoading" class="banner-crop-loading">CARGANDO IMAGEN...</div></div><div class="banner-crop-controls"><label>ZOOM GENERAL <input id="bannerCropZoom" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropZoomValue">100%</b></label><div id="bannerCropFreeControls" style="display:none;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:14px;align-items:end"><label style="min-width:0">ANCHO <input id="bannerCropScaleX" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropScaleXValue">100%</b></label><label style="min-width:0">ALTO <input id="bannerCropScaleY" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropScaleYValue">100%</b></label><div class="muted tiny" style="grid-column:1 / -1">Arrastrá la imagen para subirla, bajarla o moverla hacia los costados.</div></div><div class="banner-crop-quick"><button id="bannerCropCover" class="ghost" type="button">RELLENAR</button><button id="bannerCropContain" class="ghost" type="button">ENCAJAR</button><button id="bannerCropFree" class="ghost" type="button">LIBRE</button><button id="bannerCropCenter" class="ghost" type="button">CENTRAR</button><button id="bannerCropOriginal" class="ghost" type="button">USAR ORIGINAL</button></div></div><div class="muted tiny">Al tocar APLICAR AJUSTE, el PANEL guarda una versión optimizada del recorte. La APK no necesita cambios: recibe la imagen ya preparada.</div><div class="modal-actions"><button type="button" class="ghost" data-close>Cancelar</button><button id="bannerCropApply" class="primary" type="button" disabled>APLICAR AJUSTE</button></div><div id="bannerCropMsg" class="msg"></div>`);
  $('#modal').classList.add('banner-crop-modal');
  const canvas=$('#bannerCropCanvas'),ctx=canvas.getContext('2d'),loading=$('#bannerCropLoading'),zoom=$('#bannerCropZoom'),zoomValue=$('#bannerCropZoomValue'),apply=$('#bannerCropApply'),freeControls=$('#bannerCropFreeControls'),scaleXInput=$('#bannerCropScaleX'),scaleYInput=$('#bannerCropScaleY'),scaleXValue=$('#bannerCropScaleXValue'),scaleYValue=$('#bannerCropScaleYValue');
  ...
  const updateModeUi=()=>{if(freeControls)freeControls.style.display=mode==='free'?'grid':'none';for(const [id,key] of [['bannerCropCover','cover'],['bannerCropContain','contain'],['bannerCropFree','free']]){const b=$('#'+id);if(b)b.className=mode===key?'primary':'ghost';}};
