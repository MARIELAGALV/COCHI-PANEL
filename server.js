...
function parseMediaProviders(){
  ...
  if(!raw)return [];
  try{
    // Railway puede guardar un array, un único objeto o, por compatibilidad, una URL directa.
    let parsed;
    if(/^https?:\/\//i.test(raw)) parsed=[{name:'Catálogo Railway',url:raw,mode:'catalog',enabled:true}];
    else {
      parsed=JSON.parse(raw);
      if(!Array.isArray(parsed)) parsed=(parsed&&typeof parsed==='object')?[parsed]:[];
    }
    return parsed.map((x,i)=>({
      name:String(x?.name||x?.nombre||`Proveedor ${i+1}`).trim().slice(0,80),
      url:String(x?.url||x?.catalogUrl||x?.catalog_url||'').trim(),token:String(x?.token||'').trim(),
  ...
  }catch(e){console.warn('[MEDIA-PROVIDERS] COCHI_MEDIA_PROVIDERS_JSON inválido:',e.message);return []}
}
...
for (const [key,value] of [['demos_enabled','0'],['demo_duration_minutes','10'],['demo_blocked_categories','[]'],['adult_lock_enabled','0'],['adult_max_attempts',String(DEFAULT_ADULT_MAX_ATTEMPTS)],['adult_pin_hash',''],['playback_security_enabled','0'],['playback_generation','1'],['client_device_block_size',String(CLIENT_DEVICE_LIMIT)]]) {
...
if(getSetting('client_device_policy_migrated_v0969','0')!=='1'){
  const base=Number(getSetting('client_device_block_size',String(CLIENT_DEVICE_LIMIT)))||CLIENT_DEVICE_LIMIT;
  const rows=db.prepare('SELECT id,device_limit FROM clients').all();
  const q=db.prepare('UPDATE clients SET extra_device_allowance=?,extra_device_blocks=? WHERE id=?');
  for(const c of rows){const limit=Math.max(base,Number(c.device_limit||base)),extra=Math.max(0,limit-base),blocks=extra>0?Math.ceil(extra/base):0;q.run(extra,blocks,c.id);}
  setSetting('client_device_policy_migrated_v0969','1');
}
...
function globalClientDeviceBlockSize(){
  const n=Number(getSetting('client_device_block_size',String(CLIENT_DEVICE_LIMIT)));
  return Number.isInteger(n)&&n>=CLIENT_DEVICE_LIMIT_MIN&&n<=CLIENT_DEVICE_LIMIT_MAX?n:CLIENT_DEVICE_LIMIT;
}
function clientDeviceLimit(c){
  const raw=Number(c?.device_limit);
  const value=Number.isInteger(raw)?raw:globalClientDeviceBlockSize();
...
function clientExtraDeviceBlocks(c){const n=Number(c?.extra_device_blocks||0);return Number.isInteger(n)&&n>0?n:0;}
function clientExtraDeviceAllowance(c){const n=Number(c?.extra_device_allowance||0);return Number.isInteger(n)&&n>0?n:0;}
// v0.9.69: 1 crédito = 1 bloque global. La base global y cada ampliación cuentan como un bloque comercial.
function clientRenewCreditCost(c){return Math.max(CLIENT_CREDIT_COST,1+clientExtraDeviceBlocks(c));}
...
function extractWebCandidates(html,base){
  const found=new Map();
  // v0.9.60: normaliza URLs visibles en HTML/JS/JSON, incluidas barras escapadas y escapes unicode comunes.
  const decodeEmbedded=(raw)=>String(raw||'').replace(/\\u002[fF]/g,'/').replace(/\\u003[aA]/g,':').replace(/\\u0026/g,'&').replace(/\\\//g,'/');
  const add=(raw,kind,meta={})=>{let cleaned=decodeEmbedded(raw).replace(/&amp;/g,'&').trim();if(cleaned.startsWith('//')){try{cleaned=new URL(base).protocol+cleaned}catch{}};const url=absoluteCandidate(cleaned,base);if(!url||!/^https?:/i.test(url))return;const low=url.toLowerCase();let type=kind;if(/\.m3u8(?:[?#]|$)/i.test(low))type='HLS';else if(/\.mpd(?:[?#]|$)/i.test(low))type='DASH';else if(/\.mp4(?:[?#]|$)/i.test(low))type='MP4';else if(/\.(?:json)(?:[?#]|$)/i.test(low)||/(?:config|player|stream|live|source|manifest)/i.test(low))type=type==='SCRIPT'?'SCRIPT':type==='IFRAME'?'IFRAME':'CONFIG';const item={url,type,sourcePage:base,headers:candidateHeaders(base),...meta};if(type==='IFRAME')item.priority=iframePriority(url);if(!found.has(url))found.set(url,item);else if(type==='IFRAME'&&found.get(url).type!=='IFRAME')found.set(url,item);};
  // Iframes: no se fija un nombre ni dominio. Lee varios atributos usados por lazy-loaders y players.
  for(const m of html.matchAll(/<iframe\b([^>]*)>/gi)){
    const attrs=m[1]||'';let picked='';
    for(const a of ['src','data-src','data-lazy-src','data-url','data-player','data-frame','data-embed']){
      const q=new RegExp('(?:^|\\s)'+a+'\\s*=\\s*["\\\']([^"\\\']+)["\\\']','i').exec(attrs);
      const bare=new RegExp('(?:^|\\s)'+a+'\\s*=\\s*([^\\s>]+)','i').exec(attrs);
      const v=q?.[1]||bare?.[1]||'';if(v&&v!=='about:blank'){picked=v;break}
    }
    if(picked)add(picked,'IFRAME',{iframe:true});
  }
  // Otros recursos HTML.
  for(const m of html.matchAll(/<(source|video|audio|script)[^>]+(?:src|data-src)=['"]([^'"]+)['"]/gi)){const tag=String(m[1]||'').toLowerCase();add(m[2],tag==='script'?'SCRIPT':'MEDIA');}
  // Iframes creados o modificados por JavaScript.
  for(const m of decodeEmbedded(html).matchAll(/(?:iframe\w*\s*\.\s*src|\.setAttribute\(\s*['"]src['"]\s*,)\s*(?:=|,)??\s*['"]([^'"]+)['"]/gi))add(m[1],'IFRAME',{iframe:true,dynamicHint:true});
  for(const m of decodeEmbedded(html).matchAll(/(?:createElement\(\s*['"]iframe['"]\s*\)[\s\S]{0,500}?(?:src|data-src)\s*[:=]\s*['"]([^'"]+)['"])/gi))add(m[1],'IFRAME',{iframe:true,dynamicHint:true});
...
function rescueResolverRow(row){
  ...
  let attempts=[];try{attempts=JSON.parse(row.last_attempts_json||'[]')}catch{}
  return {...row,headers,attempts,enabled:Boolean(row.enabled)};
}
...
async function rescueFollowRedirect(rawUrl,headers={},timeoutSeconds=10){
  ...
  try{
    ...
    return {status:r.status,ok:r.ok,finalUrl:r.url||rawUrl,error:'',elapsedMs:Date.now()-started};
  }catch(e){
    return {status:null,ok:false,finalUrl:null,error:e?.name==='AbortError'?'Timeout':String(e?.message||e),elapsedMs:Date.now()-started};
  }finally{clearTimeout(timer)}
}
async function rescueFollowRemote(rawUrl,headers={},timeoutSeconds=15){
  assertPublicHttpUrl(rawUrl);
  const remoteUrl=String(process.env.COCHI_RESCUE_REMOTE_URL||'').trim();
  const secret=String(process.env.COCHI_RESCUE_REMOTE_SECRET||'').trim();
  if(!remoteUrl)return {status:null,ok:false,finalUrl:null,error:'COCHI_RESCUE_REMOTE_URL no configurada',elapsedMs:0,remote:true};
  if(!secret)return {status:null,ok:false,finalUrl:null,error:'COCHI_RESCUE_REMOTE_SECRET no configurado',elapsedMs:0,remote:true};
  try{assertPublicHttpUrl(remoteUrl)}catch(e){return {status:null,ok:false,finalUrl:null,error:'URL del Rescue Service inválida: '+e.message,elapsedMs:0,remote:true}}
  const started=Date.now(),timeoutMs=Math.max(3,Math.min(60,Number(timeoutSeconds)||15))*1000;
  const ctl=new AbortController(),timer=setTimeout(()=>ctl.abort(),timeoutMs+3000);
  try{
    const r=await fetch(remoteUrl,{method:'POST',headers:{'content-type':'application/json','authorization':'Bearer '+secret,'user-agent':`CO-CHI-PANEL/${VERSION}`},body:JSON.stringify({url:rawUrl,headers:headers||{},timeoutSeconds:Math.max(3,Math.min(60,Number(timeoutSeconds)||15))}),redirect:'error',signal:ctl.signal});
    const text=await r.text();let data={};try{data=JSON.parse(text)}catch{}
    if(!r.ok)return {status:r.status,ok:false,finalUrl:null,error:String(data.error||`Rescue remoto HTTP ${r.status}`),elapsedMs:Date.now()-started,remote:true};
    return {status:data.status??null,ok:Boolean(data.ok),finalUrl:data.finalUrl||null,error:String(data.error||''),elapsedMs:Number(data.elapsedMs||Date.now()-started),remote:true,hops:Array.isArray(data.hops)?data.hops:[]};
  }catch(e){return {status:null,ok:false,finalUrl:null,error:e?.name==='AbortError'?'Timeout del Rescue remoto':String(e?.message||e),elapsedMs:Date.now()-started,remote:true}}
  finally{clearTimeout(timer)}
}
...
async function runRescueResolverChannel(id){
  ...
  if(winner){
    ...
    const detail=last.error||(`Sin redirección válida después de ${attempts.length} intento${attempts.length===1?'':'s'}${last.status?` (último HTTP ${last.status})`:''}`);
    db.prepare(`UPDATE rescue_resolver_channels SET current_base=?,generated_url=?,last_status='ERROR',last_http=?,last_error=?,last_attempts_json=?,last_run_at=?,updated_at=? WHERE id=?`).run(lastGood,generated,last.status||null,String(detail).slice(0,1000),attemptsJson,t,t,row.id);
  }
  const out=rescueResolverRow(db.prepare('SELECT * FROM rescue_resolver_channels WHERE id=?').get(row.id));out.keptLastGood=Boolean(out.last_good_base&&out.last_status!=='OK');return out;
}
...
// v0.9.54 — los catálogos remotos también alimentan el buscador inicial.
// Así un proveedor JSON puede aportar título/póster/año y luego sus opciones de reproducción.
function providerCatalogMediaType(item,parent={}){
  const raw=(providerString(item,'media_type','mediaType','type','tipo','content_type','contentType')||providerString(parent,'media_type','mediaType','type','tipo','content_type','contentType')).toLowerCase();
  if(/series?|tv|show|episod|temporad/.test(raw))return 'tv';
  if(/movie|pel[ií]cula|film/.test(raw))return 'movie';
  if(Array.isArray(item?.seasons)||Array.isArray(item?.temporadas)||Array.isArray(item?.episodes)||Array.isArray(item?.episodios))return 'tv';
  return 'movie';
}
...
function providerCatalogOverview(item,parent={}){
  return providerString(item,'overview','description','descripcion','sinopsis','synopsis','plot')||
    providerString(parent,'overview','description','descripcion','sinopsis','synopsis','plot');
}
function providerCatalogOriginalTitle(item){return providerString(item,'original_title','originalTitle','original_name','originalName')}
function providerCatalogSearchPayload(payload,providerName,query,limit=30){
  const wanted=mediaNormTitle(query),out=[],seen=new Set();
  if(!wanted)return out;
  function addCandidate(item,parent={}){
    if(!item||typeof item!=='object'||item.adult===true)return;
    const title=providerCatalogTitle(item);if(!title)return;
    const normalized=mediaNormTitle(title);if(!normalized||(!normalized.includes(wanted)&&!wanted.includes(normalized)))return;
    // Evita mostrar categorías/grupos como si fueran películas, pero no exige que el item ya tenga
    // año/póster: muchos catálogos CO-CHI traen solo título + URI/enlaces.
    const direct=providerCatalogDirectOptions(item,parent,providerName,0);
    const hasMediaHints=Boolean(providerCatalogYear(item)||providerCatalogPoster(item,parent)||providerCatalogOverview(item,parent)||
      providerString(item,'type','tipo','media_type','mediaType','tmdb_id','tmdbId','imdb_id','imdbId','id')||
      Array.isArray(item?.seasons)||Array.isArray(item?.temporadas)||Array.isArray(item?.episodes)||Array.isArray(item?.episodios)||
      Array.isArray(item?.sources)||Array.isArray(item?.options)||Array.isArray(item?.servers)||Array.isArray(item?.streams)||Array.isArray(item?.videos));
    const containerOnly=Boolean(!direct.length&&!hasMediaHints&&
      ['samples','items','movies','peliculas','series','results','content','contenido','data','categories','categorias','groups','catalog','catalogo']
        .some(k=>item[k]&&typeof item[k]==='object'));
    if(containerOnly)return;
    const mediaType=providerCatalogMediaType(item,parent),year=providerCatalogYear(item),dedupe=`${mediaType}|${normalized}|${year}`;
    if(seen.has(dedupe))return;seen.add(dedupe);
    out.push({
      id:0,media_type:mediaType,title,original_title:providerCatalogOriginalTitle(item),year,
      poster_path:'',poster_url:providerCatalogPoster(item,parent),overview:providerCatalogOverview(item,parent),
      source:'remote-provider',provider:providerName,playback_available:direct.length>0
    });
  }
  function walk(node,parent={},depth=0){
    if(out.length>=limit||depth>10||node===null||node===undefined)return;
    if(Array.isArray(node)){for(const x of node){walk(x,parent,depth+1);if(out.length>=limit)break}return}
    if(typeof node!=='object')return;
    addCandidate(node,parent);
    // Recorremos contenedores conocidos primero y luego objetos/arrays desconocidos para tolerar JSON variados.
    const preferred=['samples','items','movies','peliculas','series','results','content','contenido','data','categories','categorias','groups','catalog','catalogo'];
    const done=new Set();
    for(const key of preferred){if(node[key]&&typeof node[key]==='object'){done.add(key);walk(node[key],node,depth+1);if(out.length>=limit)return}}
    for(const [key,value] of Object.entries(node)){
      if(done.has(key)||value===null||typeof value!=='object')continue;
      walk(value,node,depth+1);if(out.length>=limit)return;
    }
  }
...
function providerLooksLikeCatalog(provider){
  ...
  // que no termine literalmente en .json. Solo "resolver" queda fuera de la búsqueda inicial.
  try{new URL(provider.url);return provider.mode==='catalog'||provider.mode==='auto'}catch{return false}
}
async function remoteProviderCatalogSearch(query,limit=30){
  const providers=MEDIA_PROVIDERS.filter(providerLooksLikeCatalog);if(!providers.length)return [];
  const settled=await Promise.all(providers.map(async provider=>{
    try{
      const target=new URL(provider.url),payload=await providerFetchJson(provider,target);
      return providerCatalogSearchPayload(payload,provider.name,query,limit);
  ...
  for(const list of settled)for(const item of list){
    const key=`${item.media_type}|${mediaNormTitle(item.title)}|${item.year}`;if(seen.has(key))continue;seen.add(key);out.push(item);if(out.length>=limit)return out;
  }
  return out;
}
function mergeMediaSearchResults(...lists){
  ...
  for(const list of lists)for(const item of (Array.isArray(list)?list:[])){
    if(!item||!item.title)continue;const key=`${item.media_type||'movie'}|${mediaNormTitle(item.title)}|${item.year||''}`;
    if(seen.has(key))continue;seen.add(key);out.push(item);if(out.length>=30)return out;
  }
  return out;
}
function normalizeProviderOption(raw,providerName,index){
  if(!raw||typeof raw!=='object')return null;
  const url=String(raw.url||raw.uri||raw.stream||raw.file||raw.src||raw.video_url||raw.videoUrl||raw.link||raw.media_url||raw.mediaUrl||raw.movie_url||raw.movieUrl||raw.playback_url||raw.playbackUrl||'').trim();if(!/^https?:\/\//i.test(url))return null;
...
// v0.9.56 — detección de contenido por terminación multimedia, no por la URL del catálogo.
// Se inspecciona el PATH de la URL, por lo que también reconoce: pelicula.mp4?token=... o stream.m3u8?expires=...
const PROVIDER_MEDIA_EXTENSIONS=['mkv','mp4','m3u8','mpd','webm','avi','mov','m4v','ts','m2ts','mp3','aac','m4a','flac','ogg','wav'];
function providerMediaExtension(value){
  const raw=String(value||'').trim();if(!/^https?:\/\//i.test(raw))return '';
  try{
    const target=new URL(raw);let pathname='';
    try{pathname=decodeURIComponent(target.pathname||'')}catch{pathname=target.pathname||''}
    const m=pathname.match(/\.([a-z0-9]{2,8})$/i);if(!m)return '';
    const ext=String(m[1]||'').toLowerCase();return PROVIDER_MEDIA_EXTENSIONS.includes(ext)?ext:'';
  }catch{return ''}
}
function providerLooksLikeMediaUrl(value){return Boolean(providerMediaExtension(value))}
function providerFindMediaUrls(node,depth=0,out=[],seen=new Set()){
  if(depth>3||node===null||node===undefined||out.length>=50)return out;
  if(typeof node==='string'){
    const value=node.trim();if(providerLooksLikeMediaUrl(value)&&!seen.has(value)){seen.add(value);out.push(value)}
    return out;
  }
  if(Array.isArray(node)){for(const x of node){providerFindMediaUrls(x,depth+1,out,seen);if(out.length>=50)break}return out}
  if(typeof node!=='object')return out;
  // No atravesamos contenedores de otras películas/series para no adjudicar sus streams al título padre.
  const catalogContainers=new Set(['items','movies','peliculas','series','results','content','contenido','data','categories','categorias','groups','catalog','catalogo']);
  for(const [key,value] of Object.entries(node)){
    if(catalogContainers.has(String(key).toLowerCase()))continue;
    if(typeof value==='string'){
      const v=value.trim();if(providerLooksLikeMediaUrl(v)&&!seen.has(v)){seen.add(v);out.push(v)}
    }else if(value&&typeof value==='object')providerFindMediaUrls(value,depth+1,out,seen);
...
function providerCatalogTitle(item){return providerString(item,'title','name','nombre','titulo','movie','show','label','display_name','displayName','nombre_pelicula','nombrePelicula','nombre_serie','nombreSerie')}
...
function providerCatalogDirectOptions(item,parent,providerName,startIndex=0){
  const options=[],seenUrls=new Set();
  ...
  for(const key of nestedKeys){
    ...
    for(const x of arr){
      if(typeof x==='string')pushOption({...inherited,url:x});
      else if(x&&typeof x==='object')pushOption({...inherited,...x,language:providerString(x,'language','idioma','audio')||inherited.language,subtitles:providerString(x,'subtitles','subtitulos','subs')||inherited.subtitles,quality:providerString(x,'quality','calidad','qualityLabel')||inherited.quality,server:providerString(x,'server','servidor','host','name')||inherited.server});
    }
  }
  const direct=providerString(item,'stream','uri','video_url','videoUrl','file','src','url','link','media_url','mediaUrl','movie_url','movieUrl','playback_url','playbackUrl');
  if(direct)pushOption({...inherited,...item,url:direct,language:inherited.language,subtitles:inherited.subtitles,quality:inherited.quality,server:inherited.server});

  // Si el proveedor usa nombres de campos desconocidos, igualmente detectamos el stream por su extensión.
  for(const mediaUrl of providerFindMediaUrls(item))pushOption({...inherited,url:mediaUrl});
  return options;
}
...
async function fetchMediaProvider(provider,params){
  try{
    // v0.9.56: jamás usamos ".json" en la URL para decidir qué tipo de proveedor es.
    // catalog/auto primero leen la URL TAL CUAL y buscan el título + sus streams por extensión.
    if(provider.mode!=='resolver'){
      const catalogTarget=new URL(provider.url),catalogPayload=await providerFetchJson(provider,catalogTarget);
      const catalogOptions=extractCatalogOptions(catalogPayload,provider.name,params);if(catalogOptions.length)return catalogOptions;
      if(provider.mode==='catalog')return [];
    }

    // resolver, o fallback de auto: recién aquí agregamos los parámetros de la película.
    const resolverTarget=new URL(provider.url);
    for(const [k,v] of Object.entries(params))if(v!==undefined&&v!==null&&String(v)!=='')resolverTarget.searchParams.set(k,String(v));
    const resolverPayload=await providerFetchJson(provider,resolverTarget);
    return flattenProviderPayload(resolverPayload,provider.name);
...
async function route(req,res){
  ...
  // Los proveedores JSON configurados en COCHI_MEDIA_PROVIDERS_JSON aparecen en la búsqueda inicial.
  if(p==='/api/client-device/media-search'&&m==='GET'){
    ...
    // Priorizamos resultados de proveedores remotos porque son los que pueden tener reproducción directa.
    const [remoteResults,localResults]=await Promise.all([
      remoteProviderCatalogSearch(q,30),
      Promise.resolve(localCatalogSearch(q))
    ]);
    if(!TMDB_API_KEY&&!TMDB_READ_TOKEN){
      const results=mergeMediaSearchResults(remoteResults,localResults);
      return sendJson(res,200,{ok:true,query:q,provider:'cochi-remote+local',providersConfigured:MEDIA_PROVIDERS.length,remoteResults:remoteResults.length,results});
    }
    ...
    try{
      ...
      if(!r.ok){
        const results=mergeMediaSearchResults(remoteResults,localResults);
        return sendJson(res,200,{ok:true,query:q,provider:'cochi-remote+local-fallback',providersConfigured:MEDIA_PROVIDERS.length,remoteResults:remoteResults.length,results});
      }
      const payload=JSON.parse(text),items=Array.isArray(payload?.results)?payload.results:[],tmdbResults=[];
      for(const item of items){
        ...
        tmdbResults.push({
          ...
          overview:String(item?.overview||'').trim(),source:'tmdb'
        });
      }
      const results=mergeMediaSearchResults(remoteResults,localResults,tmdbResults);
      return sendJson(res,200,{ok:true,query:q,provider:'cochi-remote+local+tmdb',providersConfigured:MEDIA_PROVIDERS.length,remoteResults:remoteResults.length,results});
    }catch(e){
      const results=mergeMediaSearchResults(remoteResults,localResults);
      return sendJson(res,200,{ok:true,query:q,provider:'cochi-remote+local-fallback',providersConfigured:MEDIA_PROVIDERS.length,remoteResults:remoteResults.length,results});
    }finally{clearTimeout(timer)}
  ...
  // v0.9.54 — motor remoto: resolver API o recorrer catálogos JSON autorizados.
  ...
  if(p.startsWith('/api/admin/')){
    ...
    if(p==='/api/admin/device-policy'&&m==='PUT'){
      ...
      const b=await readJson(req),newBase=validateClientDeviceLimit(b.blockSize);if(newBase===null)return sendJson(res,400,{error:`El bloque global debe estar entre ${CLIENT_DEVICE_LIMIT_MIN} y ${CLIENT_DEVICE_LIMIT_MAX}`});
      const oldBase=globalClientDeviceBlockSize(),rows=db.prepare('SELECT id,device_limit,extra_device_allowance FROM clients').all();
      for(const c of rows){const next=newBase+Math.max(0,Number(c.extra_device_allowance||0));if(next>CLIENT_DEVICE_LIMIT_MAX)return sendJson(res,409,{error:`No se puede aplicar ${newBase}: el cliente #${c.id} superaría el máximo de ${CLIENT_DEVICE_LIMIT_MAX} dispositivos por sus ampliaciones existentes`});}
      const t=nowIso();db.exec('BEGIN');try{setSetting('client_device_block_size',String(newBase));for(const c of rows){db.prepare('UPDATE clients SET device_limit=?,updated_at=? WHERE id=?').run(newBase+Math.max(0,Number(c.extra_device_allowance||0)),t,c.id);}audit(actor.id,'global_client_device_block_changed','settings',null,`${oldBase}→${newBase}`);db.exec('COMMIT');}catch(e){db.exec('ROLLBACK');throw e;}
      return sendJson(res,200,{ok:true,oldBlockSize:oldBase,blockSize:newBase,clientsUpdated:rows.length});
    }
    ...
    if(extraDevices&&m==='POST'){
      ...
      try{
        ...
        db.prepare('UPDATE clients SET device_limit=?,extra_device_blocks=extra_device_blocks+1,extra_device_allowance=extra_device_allowance+?,updated_at=? WHERE id=?').run(newLimit,blockSize,t,c.id);
        db.prepare('INSERT INTO client_service_ledger(client_id,charged_account_id,created_by_account_id,credits_spent,previous_expiry,new_expiry,action,created_at) VALUES (?,?,?,?,?,?,?,?)').run(c.id,owner.id,actor.id,owner.role_level===1?0:1,c.expires_at||null,c.expires_at||null,'extra_devices',t);
        db.exec('COMMIT');
      }catch(e){db.exec('ROLLBACK');throw e;}
    ...
    const quickUpdateMatch=p.match(/^\/api\/admin\/content\/(tv1|tv2|movies|series)\/quick-update$/);
    if(quickUpdateMatch&&m==='POST'){
      if(actor.role_level!==1)return sendJson(res,403,{error:'Solo ADMINISTRACIÓN puede editar contenido'});
      const b=await readJson(req,30*1024*1024);
      try{
        if(!Array.isArray(b.json))throw new Error('El JSON debe ser un arreglo de categorías');
        const stats=publishEditable(quickUpdateMatch[1],b.json,actor.id,'content_quick_updated_and_published');
        return sendJson(res,200,{ok:true,stats,encrypted:true,published:true,managedUpdated:true,originalSyncRequired:true,updatedAt:nowIso()});
      }catch(e){return sendJson(res,400,{error:'No se pudo actualizar y publicar: '+e.message});}
    }
