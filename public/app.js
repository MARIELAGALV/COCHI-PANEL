...
function editContentItem(groupIndex,itemIndex=null){
  ...
  const playbackSources=(Array.isArray(cur.playbackSources)&&cur.playbackSources.length?cur.playbackSources.map(x=>({url:String(x?.url||'').trim(),headers:(x?.headers&&typeof x.headers==='object')?x.headers:{}})):[{url:String(cur.uri||'').trim(),headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{}},...legacyBackups.map(url=>({url,headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{}}))]).filter((x,i)=>x.url||i===0);
  ...
  if(isTv){renderPlaybackSources();if($('#ciAddPlaybackSource'))$('#ciAddPlaybackSource').onclick=()=>{playbackSources.push({url:'',headers:{}});renderPlaybackSources();};}
  ...
  $('#contentItemForm').onsubmit=async e=>{
    ...
    try{
      ...
      if(isTv){
        ...
        for(const row of sourceRows){const u=String(row.querySelector('.ci-playback-url')?.value||'').trim(),h=parseHeaderLines(row.querySelector('.ci-playback-headers')?.value||'');if(u)collected.push({url:u,headers:h});}
        if(!collected.length&&uri)collected.push({url:uri,headers:{}});
        if(collected.length){activePlaybackSource=Math.max(0,Math.min(activePlaybackSource,collected.length-1));obj.playbackSources=collected.map((x,i)=>({...x,enabled:i===activePlaybackSource}));obj.activePlaybackSource=activePlaybackSource;obj.uri=collected[activePlaybackSource].url;if(Object.keys(collected[activePlaybackSource].headers).length)obj.headers=collected[activePlaybackSource].headers;else delete obj.headers;}else{delete obj.playbackSources;delete obj.activePlaybackSource;delete obj.headers;}
