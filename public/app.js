...
let homeBannerPreviewSlot=1;
function homeBannerSlotInput(slot){return slot===1?$('#homeBannerMediaUrl'):$$('.home-banner-extra')[slot-2]||null}
function homeBannerSlotUrl(slot){return String(homeBannerSlotInput(slot)?.value||'').trim()}
function homeBannerAdjustedInfo(slot){const all=state.homeBannerAdjustedSlots||{};return all[String(slot)]||all[slot]||null}
function homeBannerIsAdjusted(slot){const info=homeBannerAdjustedInfo(slot),url=homeBannerSlotUrl(slot);return !!(info&&url&&String(info.sourceUrl||'')===url)}
function homeBannerPreviewUrl(slot){const url=homeBannerSlotUrl(slot);if(!url)return '';if(homeBannerIsAdjusted(slot)){const info=homeBannerAdjustedInfo(slot);return `/api/public/home-banner-image/${slot}?v=${encodeURIComponent(info.updatedAt||Date.now())}`;}return `/api/admin/image-preview?url=${encodeURIComponent(url)}`}
function renderHomeBannerSlotButtons(){
  const box=$('#homeBannerPreviewSlots');if(!box)return;
  const urls=Array.from({length:10},(_,i)=>homeBannerSlotUrl(i+1));
  box.innerHTML=urls.map((url,i)=>`<button type="button" class="${homeBannerPreviewSlot===i+1?'active':''}" data-banner-preview-slot="${i+1}" ${!url?'disabled':''}>${i+1}</button>`).join('');
  $$('[data-banner-preview-slot]').forEach(b=>b.onclick=()=>{homeBannerPreviewSlot=Number(b.dataset.bannerPreviewSlot)||1;renderHomeBannerSlotButtons();updateHomeBannerPreview();});
}
function updateHomeBannerAdjustedBadges(){
  const p=$('#homeBannerAdjustedPrimary');if(p){const ok=homeBannerIsAdjusted(1);p.textContent=ok?'AJUSTADA':'ORIGINAL';p.className=`badge ${ok?'active':'off'}`;}
...
function bindHomeBannerPreviewEvents(){
  ...
  for(const id of ids){const el=$('#'+id);if(!el||el.dataset.previewBound==='1')continue;el.dataset.previewBound='1';el.addEventListener(el.type==='checkbox'||el.tagName==='SELECT'?'change':'input',()=>{if(id==='homeBannerMediaUrl')homeBannerPreviewSlot=1;updateHomeBannerPreview();});}
...
function openHomeBannerImageEditor(slot){
  ...
  image.onload=()=>{loading.hidden=true;apply.disabled=false;setMode('cover');};
  image.onerror=()=>{loading.textContent='NO SE PUDO CARGAR LA IMAGEN';msg($('#bannerCropMsg'),'El servidor no pudo obtener esa imagen. Revisá la URL o probá otra.');};
...
async function loadHomeBanner(){
  ...
  $('#homeBannerAdjustPrimaryBtn').onclick=()=>openHomeBannerImageEditor(1);$$('[data-adjust-banner-slot]').forEach(btn=>btn.onclick=()=>openHomeBannerImageEditor(Number(btn.dataset.adjustBannerSlot)));
}
...
function editContentItem(groupIndex,itemIndex=null){
  ...
  let drmScheme=String(cur.drm_scheme||'').toLowerCase();if(!drmScheme&&String(cur.tipo||'').trim().toUpperCase()==='CLEARKEY')drmScheme='clearkey';
  ...
  const playbackSources=(Array.isArray(cur.playbackSources)&&cur.playbackSources.length?cur.playbackSources.map(x=>{let sourceDrm=String(x?.drm_scheme||'').toLowerCase();if(!sourceDrm&&String(x?.tipo||'').trim().toUpperCase()==='CLEARKEY')sourceDrm='clearkey';const sourceKeys=normalizeStoredKeys(x?.keys);if(!sourceKeys.length&&sourceDrm==='clearkey')sourceKeys.push(...normalizeStoredKeys(x?.drm_license_url));if(!sourceDrm&&sourceKeys.length)sourceDrm='clearkey';return {url:String(x?.url||'').trim(),headers:cleanPlaybackHeadersObj(x?.headers,[x?.drm_header,x?.drm_headers]),type:String(x?.type||x?.tipo||'auto').toLowerCase(),drm_scheme:sourceDrm,keys:sourceKeys,drm_license_url:sourceDrm==='widevine'?String(x?.drm_license_url||x?.license_url||'').trim():'',drm_license_headers:mergeHeaders(x?.license_headers,x?.drm_license_headers),drm_license_uri:sourceDrm==='clearkey'?String(x?.drm_license_uri||x?.clearkey_license_uri||'').trim():'',clearkey_input_order:String(x?.clearkey_input_order||x?.key_order||cur?.clearkey_input_order||cur?.key_order||'kid_key').toLowerCase()==='key_kid'?'key_kid':'kid_key',enabled:x?.enabled===true};}):[{url:String(cur.uri||'').trim(),headers:primaryPlaybackHeaders,type:streamType,drm_scheme:drmScheme,keys:existingKeyPairs.map(x=>({...x})),drm_license_url:existingLicenseUrl,drm_license_headers:existingLicenseHeaders,drm_license_uri:drmScheme==='clearkey'?String(cur?.drm_license_uri||cur?.clearkey_license_uri||'').trim():'',clearkey_input_order:String(cur?.clearkey_input_order||cur?.key_order||'kid_key').toLowerCase()==='key_kid'?'key_kid':'kid_key',enabled:true},...legacyBackups.map(url=>({url,headers:primaryPlaybackHeaders,type:'auto',drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{},clearkey_input_order:'kid_key',enabled:false}))]).filter((x,i)=>x.url||i===0);
  ...
  const templateUrlMatch=templateSourceUrl.match(/\/[^/?#]+\/([^/?#]+)\/([^/?#]+)\/(?:SA_Live_dash_(?:cenc|enc))\//i);
  if(templateUrlMatch){
  ...
  if(!templateValue&&(templateIsTemplate||templateNameRedirect||templateCodeRedirect))templateValue='{token}/live/{codigo}/{nombre}/SA_Live_dash_cenc/{nombre}.mpd';
  ...
  const routePrefill=(()=>{const m=String(templatePrefill.template||templateSourceUrl||'').match(/(?:\{token\}|https?:\/\/[^/]+)\/([^/?#{}]+)\//i);return String(m?.[1]||'live').trim()||'live';})();
  const filePrefill=(()=>{const m=String(templatePrefill.template||templateSourceUrl||'').match(/\/([^/?#]+\.mpd)(?:[?#]|$)/i);return String(m?.[1]||'{nombre}.mpd').trim()||'{nombre}.mpd';})();
  ...
  if(isTv&&$('#ciIcon')){
    const iconInput=$('#ciIcon'),preview=$('#ciIconPreview'),empty=$('#ciIconPreviewEmpty');let iconPreviewTimer=null;
    const normalizePreviewUrl=value=>{let url=String(value||'').trim();if(!url)return '';if(url.startsWith('//'))url='https:'+url;const gh=url.match(/^https?:\/\/github\.com\/([^/]+)\/([^/]+)\/blob\/([^/]+)\/(.+)$/i);if(gh)url=`https://raw.githubusercontent.com/${gh[1]}/${gh[2]}/${gh[3]}/${gh[4]}`;return url;};
    const setPreviewState=(ok)=>{if(!preview||!empty)return;preview.hidden=!ok;empty.hidden=ok;};
    const setIconPreview=()=>{const url=normalizePreviewUrl(iconInput.value);if(!preview||!empty)return;preview.dataset.originalUrl=url;preview.dataset.proxyTried='0';if(!url){preview.removeAttribute('src');setPreviewState(false);return;}setPreviewState(true);preview.src=url;};
    if(preview)preview.onerror=()=>{const url=preview.dataset.originalUrl||'';if(url&&preview.dataset.proxyTried!=='1'){preview.dataset.proxyTried='1';preview.src=`/api/admin/image-preview?url=${encodeURIComponent(url)}`;return;}preview.removeAttribute('src');setPreviewState(false);};
    if(preview)preview.onload=()=>setPreviewState(true);
    if(preview.getAttribute('src')){preview.dataset.originalUrl=normalizePreviewUrl(iconInput.value);preview.dataset.proxyTried='0';}
    iconInput.addEventListener('input',()=>{clearTimeout(iconPreviewTimer);iconPreviewTimer=setTimeout(setIconPreview,300);});
    iconInput.addEventListener('change',setIconPreview);
    setIconPreview();
  }
  ...
  const sourceKeyOrder=src=>String(src?.clearkey_input_order||src?.key_order||'kid_key').toLowerCase()==='key_kid'?'key_kid':'kid_key';
  const sourceKeysText=src=>{const order=sourceKeyOrder(src);return normalizeStoredKeys(src?.keys).map(x=>order==='key_kid'?`${x.key}:${x.kid}`:`${x.kid}:${x.key}`).join('\n');};
  const looseHeaders=parseHeadersFlexible;
  const looseKeys=(text,order='kid_key')=>String(text||'').split(/[\r\n,;]+/).map(x=>x.trim()).filter(Boolean).map(p=>{const i=p.indexOf(':');if(i<=0)return null;const left=p.slice(0,i).trim(),right=p.slice(i+1).trim();return order==='key_kid'?{kid:right,key:left}:{kid:left,key:right};}).filter(x=>x&&x.kid&&x.key);
  const syncPlaybackSourcesFromDom=()=>{
    if(!isTv)return;
    $$('[data-playback-source]').forEach(row=>{const i=Number(row.dataset.playbackSource);if(!Number.isInteger(i)||!playbackSources[i])return;const src=playbackSources[i];src.url=String(row.querySelector('.ci-playback-url')?.value||'').trim();src.headers=looseHeaders(row.querySelector('.ci-playback-headers')?.value||'');src.type=String(row.querySelector('.ci-playback-type')?.value||'auto').toLowerCase();src.drm_scheme=String(row.querySelector('.ci-playback-drm')?.value||'').toLowerCase();src.clearkey_input_order=String(row.querySelector('.ci-playback-key-order')?.value||'kid_key')==='key_kid'?'key_kid':'kid_key';src.keys=looseKeys(row.querySelector('.ci-playback-keys')?.value||'',src.clearkey_input_order);src.drm_license_uri=String(row.querySelector('.ci-playback-clearkey-uri')?.value||'').trim();src.drm_license_url=String(row.querySelector('.ci-playback-license-url')?.value||'').trim();src.drm_license_headers=looseHeaders(row.querySelector('.ci-playback-license-headers')?.value||'');});
  };
  ...
  const renderPlaybackSources=()=>{
    ...
    $$('.ci-playback-key-order').forEach(sel=>sel.onchange=()=>{const row=sel.closest('[data-playback-source]'),ta=row?.querySelector('.ci-playback-keys');if(!ta)return;ta.placeholder=sel.value==='key_kid'?'KEY:KID\\nKEY2:KID2':'KID:KEY\\nKID2:KEY2';});
    $$('.ci-playback-invert-keys').forEach(btn=>btn.onclick=()=>{const row=btn.closest('[data-playback-source]'),ta=row?.querySelector('.ci-playback-keys');if(!ta)return;ta.value=String(ta.value||'').split(/\r?\n/).map(line=>{const p=line.trim();if(!p)return line;const at=p.indexOf(':');if(at<=0||at>=p.length-1)return line;return p.slice(at+1).trim()+':'+p.slice(0,at).trim();}).join('\n');const order=row?.querySelector('.ci-playback-key-order');if(order){order.value=order.value==='key_kid'?'kid_key':'key_kid';order.dispatchEvent(new Event('change'));}ta.dispatchEvent(new Event('input',{bubbles:true}));toast('Valores ClearKey invertidos. Revisá y guardá el canal.','ok');});
  ...
  if(isTv){renderPlaybackSources();if($('#ciAddPlaybackSource'))$('#ciAddPlaybackSource').onclick=()=>{syncPlaybackSourcesFromDom();playbackSources.push({url:'',headers:{},type:'auto',drm_scheme:'',keys:[],drm_license_uri:'',drm_license_url:'',drm_license_headers:{},clearkey_input_order:'kid_key'});renderPlaybackSources();};}
  ...
  const parseKeyLines=(text,order='kid_key')=>{const out=[];for(const line of String(text||'').split(/[\r\n,;]+/)){const p=line.trim();if(!p)continue;const i=p.indexOf(':');if(i<=0||i>=p.length-1)throw new Error('Clave inválida. Usá KID:KEY o KEY:KID, una por línea.');const left=p.slice(0,i).trim(),right=p.slice(i+1).trim();if(!left||!right)throw new Error('Clave inválida. Revisá ambos valores.');out.push(order==='key_kid'?{kid:right,key:left}:{kid:left,key:right});}return out;};
      ...
      if(isTv){
        ...
        for(const row of sourceRows){
          ...
          if(sourceDrm==='clearkey'){
            const keyOrder=String(row.querySelector('.ci-playback-key-order')?.value||'kid_key')==='key_kid'?'key_kid':'kid_key';
            const keys=parseKeyLines(row.querySelector('.ci-playback-keys')?.value||'',keyOrder);
            const originalDrmUri=String(row.querySelector('.ci-playback-clearkey-uri')?.value||'').trim();
            if(!keys.length&&!/^https?:\/\//i.test(originalDrmUri))throw new Error(`ClearKey en URL ${originalIndex+1}: cargá KID:KEY/KEY:KID o conservá una URI DRM HTTP/HTTPS.`);
            src.clearkey_input_order=keyOrder;
            src.keys=keys;
            if(originalDrmUri)src.drm_license_uri=originalDrmUri;
        ...
        delete obj.drm_scheme;delete obj.keys;delete obj.drm_license_uri;delete obj.clearkey_license_uri;delete obj.drm_license_url;delete obj.drm_license_headers;delete obj.license_url;delete obj.license_headers;
        if(active.drm_scheme==='clearkey'){obj.drm_scheme='clearkey';obj.keys=active.keys;if(active.drm_license_uri)obj.drm_license_uri=active.drm_license_uri;obj.clearkey_input_order=active.clearkey_input_order==='key_kid'?'key_kid':'kid_key';}
        ...
        if(useTemplate){
          const nameRedirect=String($('#ciNameRedirect')?.value||'').trim(),codeRedirect=String($('#ciCodeRedirect')?.value||'').trim();
          const routeRedirect=String($('#ciRouteRedirect')?.value||'').trim(),fileRedirect=String($('#ciFileRedirect')?.value||'').trim();
          let template=String($('#ciTemplate')?.value||'').trim();
          if(!nameRedirect||!codeRedirect||!routeRedirect||!fileRedirect)throw new Error('Plantilla CO-CHI: completá nameRedirect, codeRedirect, ruta y archivo final.');
          const existingDash=(template.match(/SA_Live_dash_(?:cenc|enc)/i)||[])[0];
          const dash=/^live$/i.test(routeRedirect)?'SA_Live_dash_cenc':(/^out$/i.test(routeRedirect)?'SA_Live_dash_enc':(existingDash||'SA_Live_dash_enc'));
          template='{token}/'+routeRedirect+'/{codigo}/{nombre}/'+dash+'/'+fileRedirect;
