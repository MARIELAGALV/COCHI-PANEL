...
function editContentItem(groupIndex,itemIndex=null){
  ...
  const playbackSources=(Array.isArray(cur.playbackSources)&&cur.playbackSources.length?cur.playbackSources.map(x=>({url:String(x?.url||'').trim(),headers:(x?.headers&&typeof x.headers==='object')?x.headers:{},drm_scheme:String(x?.drm_scheme||'').toLowerCase(),keys:Array.isArray(x?.keys)?x.keys:[],drm_license_url:String(x?.drm_license_url||x?.license_url||'').trim(),drm_license_headers:(x?.drm_license_headers&&typeof x.drm_license_headers==='object')?x.drm_license_headers:((x?.license_headers&&typeof x.license_headers==='object')?x.license_headers:{})})):[{url:String(cur.uri||'').trim(),headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{},drm_scheme:drmScheme,keys:Array.isArray(cur.keys)?cur.keys:[],drm_license_url:existingLicenseUrl,drm_license_headers:existingLicenseHeaders},...legacyBackups.map(url=>({url,headers:(cur.headers&&typeof cur.headers==='object')?cur.headers:{},drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{}}))]).filter((x,i)=>x.url||i===0);
  let activePlaybackSource=Number.isInteger(cur.activePlaybackSource)?cur.activePlaybackSource:0;if(activePlaybackSource<0||activePlaybackSource>=playbackSources.length)activePlaybackSource=0;
  ...
  const renderPlaybackSources=()=>{
    ...
    $('#ciPlaybackSources').innerHTML=playbackSources.map((src,i)=>{const drm=String(src.drm_scheme||'').toLowerCase();return `<div class="playback-source-row" data-playback-source="${i}"><div class="playback-source-row-head"><strong>URL ${i+1}${i===0?' · PRINCIPAL':' · SOPORTE'}</strong><label class="switch-row playback-active-label"><input type="radio" name="ciActivePlaybackSource" value="${i}" ${i===activePlaybackSource?'checked':''}> ACTIVA</label>${i>0?`<button type="button" class="danger small-btn" data-remove-playback-source="${i}">ELIMINAR</button>`:''}</div><label>URL ${i+1}<textarea class="ci-playback-url" rows="2" spellcheck="false" placeholder="https://...">${esc(src.url||'')}</textarea></label><label>Headers de URL ${i+1}<textarea class="ci-playback-headers" rows="4" spellcheck="false" placeholder="Referer: https://...&#10;Origin: https://...&#10;User-Agent: ...">${esc(headerText(src.headers))}</textarea></label><label>DRM de URL ${i+1}<select class="ci-playback-drm"><option value="" ${!drm?'selected':''}>Sin DRM</option><option value="clearkey" ${drm==='clearkey'?'selected':''}>ClearKey / MultiKey</option><option value="widevine" ${drm==='widevine'?'selected':''}>Widevine</option></select></label><div class="ci-source-clearkey" style="display:${drm==='clearkey'?'grid':'none'}"><label>Claves ClearKey / MultiKey de URL ${i+1}<textarea class="ci-playback-keys" rows="4" spellcheck="false" placeholder="KID:KEY&#10;KID2:KEY2">${esc(sourceKeysText(src))}</textarea></label></div><div class="ci-source-widevine" style="display:${drm==='widevine'?'grid':'none'}"><label>Licencia Widevine de URL ${i+1}<textarea class="ci-playback-license-url" rows="2" spellcheck="false" placeholder="https://licencias.ejemplo.com/widevine">${esc(src.drm_license_url||'')}</textarea></label><label>Headers de licencia Widevine de URL ${i+1}<textarea class="ci-playback-license-headers" rows="4" spellcheck="false" placeholder="Authorization: Bearer ...&#10;Origin: https://...">${esc(headerText(src.drm_license_headers))}</textarea></label></div></div>`}).join('');
    $$('input[name="ciActivePlaybackSource"]').forEach(r=>r.onchange=()=>{activePlaybackSource=Number(r.value)||0;});
    ...
    $$('[data-remove-playback-source]').forEach(b=>b.onclick=()=>{const i=Number(b.dataset.removePlaybackSource);playbackSources.splice(i,1);if(activePlaybackSource===i)activePlaybackSource=0;else if(activePlaybackSource>i)activePlaybackSource--;renderPlaybackSources();});
  };
  if(isTv){renderPlaybackSources();if($('#ciAddPlaybackSource'))$('#ciAddPlaybackSource').onclick=()=>{playbackSources.push({url:'',headers:{},drm_scheme:'',keys:[],drm_license_url:'',drm_license_headers:{}});renderPlaybackSources();};}
  ...
  $('#contentItemForm').onsubmit=async e=>{
    ...
    try{
      ...
      if(isTv){
        const t=$('#ciStreamType').value||'auto',drm=$('#ciDrmScheme').value||'';
        ...
        for(const row of sourceRows){const u=String(row.querySelector('.ci-playback-url')?.value||'').trim(),h=parseHeaderLines(row.querySelector('.ci-playback-headers')?.value||''),sourceDrm=String(row.querySelector('.ci-playback-drm')?.value||'').trim();if(!u)continue;const src={url:u,headers:h,drm_scheme:sourceDrm};if(sourceDrm==='clearkey'){const keys=parseKeyLines(row.querySelector('.ci-playback-keys')?.value||'');if(!keys.length)throw new Error(`ClearKey en URL ${collected.length+1}: cargá al menos un par KID:KEY.`);src.keys=keys;}else if(sourceDrm==='widevine'){const licenseUrl=String(row.querySelector('.ci-playback-license-url')?.value||'').trim(),licenseHeaders=parseHeaderLines(row.querySelector('.ci-playback-license-headers')?.value||'');if(!/^https?:\/\//i.test(licenseUrl))throw new Error(`Widevine en URL ${collected.length+1}: cargá una URL de licencia HTTP/HTTPS válida.`);src.drm_license_url=licenseUrl;if(Object.keys(licenseHeaders).length)src.drm_license_headers=licenseHeaders;}collected.push(src);}
        if(!collected.length&&uri)collected.push({url:uri,headers:{},drm_scheme:''});
        if(collected.length){activePlaybackSource=Math.max(0,Math.min(activePlaybackSource,collected.length-1));obj.playbackSources=collected.map((x,i)=>({...x,enabled:i===activePlaybackSource}));obj.activePlaybackSource=activePlaybackSource;const active=collected[activePlaybackSource];obj.uri=active.url;if(Object.keys(active.headers).length)obj.headers=active.headers;else delete obj.headers;delete obj.drm_scheme;delete obj.keys;delete obj.drm_license_url;delete obj.drm_license_headers;delete obj.license_url;delete obj.license_headers;if(active.drm_scheme==='clearkey'){obj.drm_scheme='clearkey';obj.keys=active.keys;}else if(active.drm_scheme==='widevine'){obj.drm_scheme='widevine';obj.drm_license_url=active.drm_license_url;if(active.drm_license_headers&&Object.keys(active.drm_license_headers).length)obj.drm_license_headers=active.drm_license_headers;}}else{delete obj.playbackSources;delete obj.activePlaybackSource;delete obj.headers;}
