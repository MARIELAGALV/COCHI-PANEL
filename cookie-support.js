...
async function fetchCookieFromUrl({url,sourceUrl,headers,currentCookie}){
  ...
  let status=0,finalUrl=current.toString();
  ...
  if(!received.size)throw new Error('La URL no devolvio ningun Set-Cookie util para esta fuente');
  return {cookie:serializeCookieMap(jar),received:[...received],status,finalUrl};
}
