...
function openHomeBannerImageEditor(slot){
  ...
  openModal(`<div class="banner-crop-head"><div><h3>Ajustar imagen ${slot}</h3><p class="muted small">Mové la imagen con el mouse o el dedo. En <b>LIBRE</b> podés cambiar ancho y alto por separado y seguir viendo toda la imagen mientras ajustás.</p></div><span class="badge">16:9</span></div><div class="banner-crop-stage" style="width:min(100%,98vh);margin:0 auto"><canvas id="bannerCropCanvas" width="1280" height="720"></canvas><div id="bannerCropLoading" class="banner-crop-loading">CARGANDO IMAGEN...</div></div><div class="banner-crop-controls" style="display:grid;grid-template-columns:1fr;gap:12px;margin-top:12px"><div id="bannerCropAxes" style="display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:14px;align-items:end"><label style="display:grid;grid-template-columns:1fr auto;gap:5px 10px;align-items:center;margin:0;min-width:0"><span>ZOOM GENERAL</span><b id="bannerCropZoomValue">100%</b><input id="bannerCropZoom" type="range" min="35" max="300" step="1" value="100" style="grid-column:1 / -1"></label><label id="bannerCropWidthControl" style="display:none;grid-template-columns:1fr auto;gap:5px 10px;align-items:center;margin:0;min-width:0"><span>ANCHO</span><b id="bannerCropScaleXValue">100%</b><input id="bannerCropScaleX" type="range" min="35" max="300" step="1" value="100" style="grid-column:1 / -1"></label><label id="bannerCropHeightControl" style="display:none;grid-template-columns:1fr auto;gap:5px 10px;align-items:center;margin:0;min-width:0"><span>ALTO</span><b id="bannerCropScaleYValue">100%</b><input id="bannerCropScaleY" type="range" min="35" max="300" step="1" value="100" style="grid-column:1 / -1"></label></div><div class="banner-crop-quick" style="justify-content:center"><button id="bannerCropCover" class="ghost" type="button">RELLENAR</button><button id="bannerCropContain" class="ghost" type="button">ENCAJAR</button><button id="bannerCropFree" class="ghost" type="button">LIBRE</button><button id="bannerCropCenter" class="ghost" type="button">CENTRAR</button><button id="bannerCropOriginal" class="ghost" type="button">USAR ORIGINAL</button></div><div class="muted tiny" style="text-align:center">En LIBRE: arrastrá la imagen para subirla, bajarla o moverla hacia los costados.</div></div><div class="muted tiny">Al tocar APLICAR AJUSTE, el PANEL guarda una versión optimizada del recorte. La APK no necesita cambios: recibe la imagen ya preparada.</div><div class="modal-actions"><button type="button" class="ghost" data-close>Cancelar</button><button id="bannerCropApply" class="primary" type="button" disabled>APLICAR AJUSTE</button></div><div id="bannerCropMsg" class="msg"></div>`);
  $('#modal').classList.add('banner-crop-modal');
  const canvas=$('#bannerCropCanvas'),ctx=canvas.getContext('2d'),loading=$('#bannerCropLoading'),zoom=$('#bannerCropZoom'),zoomValue=$('#bannerCropZoomValue'),apply=$('#bannerCropApply'),axes=$('#bannerCropAxes'),widthControl=$('#bannerCropWidthControl'),heightControl=$('#bannerCropHeightControl'),scaleXInput=$('#bannerCropScaleX'),scaleYInput=$('#bannerCropScaleY'),scaleXValue=$('#bannerCropScaleXValue'),scaleYValue=$('#bannerCropScaleYValue');
  ...
  const updateModeUi=()=>{
    ...
    if(widthControl)widthControl.style.display=free?'grid':'none';
    if(heightControl)heightControl.style.display=free?'grid':'none';
    if(axes)axes.style.gridTemplateColumns=free?'repeat(3,minmax(0,1fr))':'1fr';
  ...
  const resetFactors=()=>{zoomFactor=1;scaleX=1;scaleY=1;offsetX=0;offsetY=0;zoom.value='100';zoomValue.textContent='100%';scaleXInput.value='100';scaleYInput.value='100';scaleXValue.textContent='100%';scaleYValue.textContent='100%';};
  ...
  zoom.oninput=()=>{zoomFactor=Math.max(.35,Number(zoom.value||100)/100);zoomValue.textContent=`${Math.round(zoomFactor*100)}%`;draw();};
  scaleXInput.oninput=()=>{if(mode!=='free')mode='free';scaleX=Math.max(.35,Number(scaleXInput.value||100)/100);scaleXValue.textContent=`${Math.round(scaleX*100)}%`;updateModeUi();draw();};
  scaleYInput.oninput=()=>{if(mode!=='free')mode='free';scaleY=Math.max(.35,Number(scaleYInput.value||100)/100);scaleYValue.textContent=`${Math.round(scaleY*100)}%`;updateModeUi();draw();};
