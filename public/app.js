...
function openHomeBannerImageEditor(slot){
  ...
  openModal(`<div class="banner-crop-head"><div><h3>Ajustar imagen ${slot}</h3><p class="muted small">Mové la imagen con el mouse o el dedo. En <b>LIBRE</b> podés cambiar ancho y alto por separado y seguir viendo toda la imagen mientras ajustás.</p></div><span class="badge">16:9</span></div><div class="banner-crop-stage" style="width:min(100%,98vh);margin:0 auto"><canvas id="bannerCropCanvas" width="1280" height="720"></canvas><div id="bannerCropLoading" class="banner-crop-loading">CARGANDO IMAGEN...</div></div><div class="banner-crop-controls" style="display:grid;grid-template-columns:1fr;gap:12px;margin-top:12px"><div id="bannerCropAxes" style="display:grid;grid-template-columns:1fr;gap:12px;align-items:end"><label style="display:grid;grid-template-columns:auto minmax(120px,1fr) auto;gap:10px;align-items:center;margin:0;min-width:0">ZOOM GENERAL <input id="bannerCropZoom" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropZoomValue">100%</b></label><label id="bannerCropWidthControl" style="display:none;grid-template-columns:auto minmax(120px,1fr) auto;gap:10px;align-items:center;margin:0;min-width:0">ANCHO <input id="bannerCropScaleX" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropScaleXValue">100%</b></label><label id="bannerCropHeightControl" style="display:none;grid-template-columns:auto minmax(120px,1fr) auto;gap:10px;align-items:center;margin:0;min-width:0">ALTO <input id="bannerCropScaleY" type="range" min="35" max="300" step="1" value="100"><b id="bannerCropScaleYValue">100%</b></label></div><div class="banner-crop-quick" style="justify-content:center"><button id="bannerCropCover" class="ghost" type="button">RELLENAR</button><button id="bannerCropContain" class="ghost" type="button">ENCAJAR</button><button id="bannerCropFree" class="ghost" type="button">LIBRE</button><button id="bannerCropCenter" class="ghost" type="button">CENTRAR</button><button id="bannerCropOriginal" class="ghost" type="button">USAR ORIGINAL</button></div><div class="muted tiny" style="text-align:center">En LIBRE: arrastrá la imagen para subirla, bajarla o moverla hacia los costados.</div></div><div class="muted tiny">Al tocar APLICAR AJUSTE, el PANEL guarda una versión optimizada del recorte. La APK no necesita cambios: recibe la imagen ya preparada.</div><div class="modal-actions"><button type="button" class="ghost" data-close>Cancelar</button><button id="bannerCropApply" class="primary" type="button" disabled>APLICAR AJUSTE</button></div><div id="bannerCropMsg" class="msg"></div>`);
  ...
  const updateModeUi=()=>{
    ...
    if(axes)axes.style.gridTemplateColumns=free?(window.innerWidth<680?'1fr':'repeat(3,minmax(0,1fr))'):'1fr';
